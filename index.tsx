      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8 border-b border-gray-100 dark:border-gray-700 pb-8">
          <div className="col-span-1">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">Setting the pace for media reporting</p>
          </div>
          <div>
        ...
        {tab === 'ads' && ads.map((a:any) => (
            <div key={a.id} className="bg-white p-4 rounded shadow mb-4 border-l-4 border-yellow-400">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                    <div>
                        <p className="text-[10px] font-bold">Receipt</p>
                        {a.receiptImage ? (
                            <a href={a.receiptImage} target="_blank" rel="noreferrer">
                                <img src={a.receiptImage} className="h-24 object-cover border w-full rounded hover:opacity-80 transition-opacity" />
                            </a>
                        ) : <span className="text-xs text-gray-400">No Receipt</span>}
                    </div>
                    <div>
                        <p className="text-[10px] font-bold">Ad Creative</p>
                        {a.adImage ? (
                            <a href={a.adImage} target="_blank" rel="noreferrer">
                                <img src={a.adImage} className="h-24 object-cover border w-full rounded hover:opacity-80 transition-opacity" />
                            </a>
                ...
                <div className="flex gap-2 mt-2">
                    {/* Fix: Ensure status check is case-insensitive */}
                    {a.status && a.status.toLowerCase() === 'pending' && (
                        <>
                            <button onClick={()=>onApproveAd(a.id)} className="bg-green-500 text-white px-3 py-2 rounded text-xs flex-1 font-bold hover:bg-green-600">Approve</button>
                            <button onClick={()=>onRejectAd(a.id)} className="bg-red-500 text-white px-3 py-2 rounded text-xs flex-1 font-bold hover:bg-red-600">Reject</button>
                        </>
                    )}
                ...
                {a.status && a.status.toLowerCase() === 'active' && <span className="text-green-600 font-bold text-xs flex items-center gap-1 mt-2"><CheckCircle className="w-3 h-3"/> Active</span>}
            </div>
        ))}
